...
BOOL						g_bHideFrustum = TRUE;
BOOL						g_bLockFrustum = FALSE;
...
void LogStatus()
{
	ZFLog::GetInstance()->Log("Wireframe:%d", g_bWireframe);
	ZFLog::GetInstance()->Log("HideFrustum:%d", g_bHideFrustum);
	ZFLog::GetInstance()->Log("LockFrustum:%d", g_bLockFrustum);
}
...
HRESULT ProcessFrustumCull()
{
	...
	if (FAILED(g_pIB->Lock(0, (g_cxHeight - 1) * (g_cxHeight - 1) * 2 * sizeof(MYINDEX), (void**)&pI, 0)))
		return E_FAIL;
	...
	for (DWORD z = 0; z < g_czHeight - 1; z++)
	{
		for (DWORD x = 0; x < g_cxHeight - 1; x++)
		{
			...
			i[3] = ((z + 1)*g_cxHeight + x + 1);

			b[0] = ZFrustum::GetInstance()->IsIn(&g_pvHeightMap[i[0]]);
			b[1] = ZFrustum::GetInstance()->IsIn(&g_pvHeightMap[i[1]]);
			b[2] = ZFrustum::GetInstance()->IsIn(&g_pvHeightMap[i[2]]);
			if (b[0] | b[1] | b[2])
			{
				idx._0 = i[0];
				idx._1 = i[1];
			...
			b[2] = ZFrustum::GetInstance()->IsIn(&g_pvHeightMap[i[2]]);
			b[1] = ZFrustum::GetInstance()->IsIn(&g_pvHeightMap[i[1]]);
			b[3] = ZFrustum::GetInstance()->IsIn(&g_pvHeightMap[i[3]]);
			if (b[2] | b[1] | b[3])
			{
				idx._0 = i[2];
				idx._1 = i[1];
...
VOID Animate()
{
	...
	D3DXMATRIXA16 m;
	D3DXMATRIXA16 *pView;
	pView = ZCamera::GetInstance()->GetViewMatrix();
	m = *pView * g_matProj;
	if (!g_bLockFrustum)
		ZFrustum::GetInstance()->Make(&m);
	ProcessFrustumCull();
...
void Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		if (!g_bHideFrustum)
			ZFrustum::GetInstance()->Draw(g_pd3dDevice);
